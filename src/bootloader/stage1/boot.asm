...
start:
    ...
    ; some BIOSes might start us at 07C0:0000 instead of 0000:7C00, make sure we are in
...
.root_dir_after:
    ...
    mov dl, [ebr_drive_number]      ; dl = drive num (saved previously)
...
.load_kernel_loop:
    ...
    ; hard coded value
    add ax, 31                      ; first cluster = (cluster num - 2) * sectors_per_cluster + start_sector
...
.read_finish:
    ...
    mov ax, KERNEL_LOAD_SEGMENT     ; set segment regs
    mov ds, ax
    mov es, ax

    jmp KERNEL_LOAD_SEGMENT:KERNEL_LOAD_OFFSET

    jmp wait_key_and_reboot         ; if this occurs something went wrong ;(
